...
import { UserCircleIcon } from './icons/NavIcons';

interface AuthPageProps {
  language: Language;
  onLoginSuccess: (response: { user: Customer | Vendor; token: string }) => void;
...
const AuthPage: React.FC<AuthPageProps> = ({ language, onLoginSuccess, onSignupSuccess, onExit, initialDetails }) => {
  const [step, setStep] = useState<AuthStep>(initialDetails?.phone ? 'otp' : 'phone');
  ...
  useEffect(() => {
    const sendInitialOtp = async () => {
        if (initialDetails?.phone && step === 'otp') {
            setIsLoading(true);
            try {
                const { userExists } = await api.requestOtp(initialDetails.phone);
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('pending review')) {
          setError('Your application is still pending review. Please check back later.');
      } else {
          setError('Invalid OTP or unknown error occurred.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
        ...
        setError('');
        try {
            const url = await api.uploadFileToS3(file, { phone, otp: otp.join('') });
            setAvatarUrl(url);
        } catch (err) {
            console.error(err);
            ...
            <form onSubmit={handleDetailsSubmit} className="space-y-4">
              <div className="flex justify-center">
                  <label htmlFor="avatar-upload" className="cursor-pointer">
                      {avatarUrl ? (
                         <img src={avatarUrl} alt="Avatar" className="w-24 h-24 rounded-full object-cover border-4 border-slate-200 hover:border-primary transition" />
                      ) : (
                        <div className="w-24 h-24 rounded-full border-4 border-slate-200 hover:border-primary transition flex items-center justify-center bg-slate-100">
                            <UserCircleIcon className="w-20 h-20 text-slate-400" />
                        </div>
                      )}
                      <input id="avatar-upload" type="file" className="hidden" accept="image/*" onChange={handleAvatarChange} disabled={isLoading} />
                  </label>
              </div>
